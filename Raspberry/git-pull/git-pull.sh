...
do
	...
	if [[ "$?" -eq 0 ]]; then
		...
		if [[ "$pull" =~ "up to date" ]]; then
			echo No changes
			cloned=-1
		# If pull changed Arduino files
		else
		    if [[ "$pull" =~ "Arduino/printy.ino/" ]]; then
...
if [[ "$cloned" -gt 0 ]]; then
    ...
    if [[ "$cloned%2" -eq 1 ]] || [[ "`cat /home/pi/Printy-McPrintface/Raspberry/git-pull/.lastCompile`" -ne "0" ]]; then
